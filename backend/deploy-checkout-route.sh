#!/bin/bash
# Script para fazer deploy da rota de checkout atualizada
...
USER="root"
FILE="routes/checkout.routes.js"
...
# Conectar via SSH e fazer o deploy
...
if ! grep -q "import Stripe" routes/checkout.routes.js; then
    echo "⚠️  Aviso: Verifique se o Stripe está instalado no servidor"
fi
...
if command -v pm2 > /dev/null 2>&1 && pm2 list 2>/dev/null | grep -q backend; then
    ...
    echo "   Reinicie manualmente: pm2 restart backend"
fi
